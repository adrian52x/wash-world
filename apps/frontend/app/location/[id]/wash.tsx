...
export default function WashProcessScreen() {
  ...
  const [selectedWash, setSelectedWash] = useState(membershipWash || onlyAutoWashTypes[1] || null);
        ...
        <View className="flex-1 p-6">
          ...
          {userSession?.userMembership?.membership && (
            <View className="flex-row items-center gap-2 mb-4 ">
              <Text className="font-subheader text-bodyText text-body">Membership:</Text>
          ...
          <View className="flex-row items-center gap-2 mb-4 ">
            <Text className="font-subheader text-bodyText text-body">License plate scanned:</Text>
            <Text className="font-subheader text-bodyText underline text-green-light">
          ...
          <View className="flex-col flex-1">
            {onlyAutoWashTypes.map((wash) => {
              ...
              return (
                <TouchableOpacity
                  key={wash.type}
                  className={`flex-col px-2 py-3 border border-gray-300 ${isSelected ? 'bg-green-light' : 'bg-white'}`}
  ...
  if (processIndex < processSteps.length) {
    ...
    return (
      <View className="flex-1 pt-[100px] items-center bg-white">
        ...
        {current.image && <Image source={current.image} className="w-48 h-48 mb-6" resizeMode="contain" />}
        <TouchableOpacity className="bg-green-light px-8 py-3 mt-10" onPress={() => setStep(step + 1)}>
  ...
  return (
    <View className="flex-1 pt-[200px] items-center bg-green-light">
      ...
      <Text className="text-bodyText font-subheader text-white">You can now exit the hall.</Text>
      <Image source={CarImage} className="w-48 h-48 mb-2" resizeMode="contain" />
      <TouchableOpacity className="bg-accent-gray-80 px-8 py-3" onPress={() => router.push(`/(tabs)`)}>
