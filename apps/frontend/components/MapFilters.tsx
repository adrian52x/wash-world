...
export const MapFilters: React.FC<MapFiltersProps> = ({ filter, setFilter }) => (
  <View className="absolute left-[20px] right-[20px] top-[10px] z-10 flex-row">
    ...
    >
        <Text className={`${filter === FILTERS.AUTO ? 'text-white' : 'text-green-500'} font-semibold text-center`}>Auto Wash</Text>
    </TouchableOpacity>
    ...
    >
        <Text className={`${filter === FILTERS.SELF ? 'text-white' : 'text-green-500'} font-semibold text-center`}>Self Wash</Text>
    </TouchableOpacity>
  </View>
);